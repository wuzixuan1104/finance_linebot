...
class Validation {
  ...
  private function _isEmail ($msg = '') {
    is_string ($this->val) && filter_var ($this->val, FILTER_VALIDATE_EMAIL) || Validation::error ($this->title . ($msg ? $msg : '需要是 E-Mail 格式'));
  }
  ...
  private function _isDate ($msg = null) {
    is_string ($this->val) || Validation::error ($this->title . ($msg ? $msg : '格式必須是字串'));
    // $this->doTrim ();
    DateTime::createFromFormat ('Y-m-d', $this->val) !== false || Validation::error ($this->title . ($msg ? $msg : '格式必須是 Date'));
  }
