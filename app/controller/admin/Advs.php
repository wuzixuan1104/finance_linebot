...
class Advs extends AdminRestfulController {

  public function __construct () {
    parent::__construct ();

    $this->layout->with ('title', '廣告列表')
  ...
  public function index() {
    ...
    $search = Restful\Search::create ($where)
                            ...
                            ->select ('類型', function ($val) { return Where::create ('type = ?',  $val ); },
                              isset( Adv::$typeTexts ) ? array_map( function($value, $text) {
                                return array( 'text' => $text, 'value' => $value);
                              }, array_keys(Adv::$typeTexts), Adv::$typeTexts ): array())
                            ->select ('審核狀態', function ($val) { return Where::create ('review = ?',  $val ); },
                              isset( Adv::$reviewTexts ) ? array_map( function($value, $text) {
                                return array( 'text' => $text, 'value' => $value);
                              }, array_keys(Adv::$reviewTexts), Adv::$reviewTexts ): array())
                            ;

    $total = Adv::count ($where);
    $page  = Pagination::info ($total);
    $objs  = Adv::find ('all', array (
               'include' => array ('user'),
               'offset' => $page['offset'],
               'limit' => $page['limit'],
               'where' => $where));
    ...
    return $this->view->setPath('admin/Advs/index.php')
  ...
  public function show($obj) {
    return $this->view->setPath('admin/Advs/show.php');
  }
