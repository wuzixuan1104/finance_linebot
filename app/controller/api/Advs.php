...
class Advs extends ApiLoginController {
  ...
  public function index () {
    ...
    $advs = array_map (function ($adv) {
      ...
      return $adv;
    }, Adv::find ('all', array ('offset' => $gets['offset'], 'limit' => $gets['limit'])));
  ...
  public function create() {
    $validation = function (&$posts, $files) {
      ...
      if( $posts['brand_product_id'] && !BrandProduct::find_by_id($posts['brand_product_id']) )
        Validation::error('查無此品牌商品');

      $posts['user_id'] = $this->user->id;
      $posts['review'] = Adv::REVIEW_FAIL;
      $posts['type'] = Adv::TYPE_PICTURE;
      $posts['cnt_like'] = 0;
      $posts['cnt_view'] = 0;
      $posts['cnt_message'] = 0;
    ...
    $transaction = function ($posts) {
      ...
      foreach( $posts['details'] as $detail ) {
        if ( !$detailObj = AdvDetail::create(array_merge($detail, array(
          'adv_id' => $obj->id, 'link' => '', 'pic' => '', 'file' => '') ) ) )
          return false;
