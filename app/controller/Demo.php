...
class Demo extends Controller {
  ...
  public function index () {
    ...
    $brands = array_map (function ($user) {
      ...
      array_map (function ($brand) {
        ...
        array_map (function ($product) use ($brand) {
          ...
          return Demo::createAdvOthers ($adv);
    ...
    $receives = array_map (function ($receive) {
      ...
      if (!$bonusReceive = $user->receive (Account::find ('one', array ('order' => 'id ASC', 'where' => array ('user_id = ?', $user->id))), $price, BonusReceive::TYPE_ATM))
        echo "GG~~~~~~~~~~\n";
      else
