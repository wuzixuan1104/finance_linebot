...
abstract class Row {
  ...
  public function getValue ($name, $value, $column = null) {
    if ($value !== null || !$this->obj) return $value;
...
abstract class Input extends Row {
  ...
  protected function setType ($type) { $type && is_string ($type) && in_array ($type, array ('text', 'color', 'number', 'date', 'email', 'password')) && $this->type = $type; return $this; }
...
class Checkboxs extends Row {
  ...
  public static function create ($title, $name, $values = null) { return (new static ())->setTitle ($title)->setName ($name)->setValues ($values); }
  public static function need ($title, $name, $values = null) { return self::create ($title, $name, $values)->setNeed (true); }
  public static function maybe ($title, $name, $values = null) { return self::create ($title, $name, $values)->setNeed (false); }
