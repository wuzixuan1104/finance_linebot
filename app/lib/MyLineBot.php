...
class MyLineBotLog {
  private $source, $event, $param;

  public function __construct($source, $event) {
    $this->source = $source;
    $this->event = $event;
    $this->param = [];
  }

  public static function init( $source, $event ) {
    return new MyLineBotLog($source, $event);
  }
  ...
  private function textMessage() {
    ...
    transaction( function() use ($param, &$obj) { return $obj = \M\Text::create($param); });
    return $obj;
  }
...
class RichMenuGenerator {
  public static function create4user($sid) {
    if(!RichMenu::linkToUser($sid, 'richmenu-3e6069bad5fc3ff341050fc0ca6683b9'))
      return false;
    return true;
  }
  ...
  public static function create() {
    ...
    if(!$richMenuId = RichMenu::create(BuildRichMenu::create(
      BuildRichMenu::size(843), true, '摳摳v6', '更多',
      [ 
    ...
    if(!$img = RichMenu::uploadImage($richMenuId, '/Users/wu-tzu-hsuan/www/linebot/finance_linebot_v2/asset/img/menu_v5.png', 'image/png'))
      return false;
